...
import {
  ArrowLeft, ArrowRight, ScrollText, Mail, Phone, Globe, User2, MessageCircle,
} from 'lucide-react';
...
function TcList({ format, items }) {
  if (format === 'decimal') {
    return (
      <ol className="space-y-2.5 list-decimal list-outside pl-5 marker:text-[#E8A020] marker:font-700 mb-4">
        {items.map((spans, i) => (
          <li key={i} className="font-nunito text-[#2D2520]/75 text-base leading-relaxed pl-1">
            <Spans spans={spans} />
          </li>
        ))}
      </ol>
...
function TcBlock({ block }) {
  ...
  if (block.kind === 'subheading') {
    return (
      <h3 id={`section-${block.num}`} className="scroll-mt-28 font-fredoka text-[#2D2520] text-lg sm:text-xl mt-8 mb-3">
        <span className="text-[#E8A020]">{block.num}</span> {block.title}
      </h3>
    );
  }
  if (block.kind === 'list') {
    return <TcList format={block.format} items={block.items} />;
  }
  return null;
}
  ...
  return (
    <section id={`section-${section.num}`} className={`scroll-mt-28 py-10 ${isFirst ? '' : 'border-t border-[#2D2520]/10'}`}>
      ...
      <div className="bg-[#F5F3EE]/60 rounded-2xl p-6 sm:p-8">
        ...
        <p className="font-nunito text-[#2D2520]/45 text-xs font-700 uppercase tracking-wide mb-4">Trading as Rook Foundations</p>
    ...
    <div className="bg-[#FAFAF7] pt-32 pb-24">
      ...
      <section className="relative overflow-hidden py-16">
        ...
        <div className="max-w-3xl mx-auto px-6 lg:px-12 relative z-10">
          ...
          <p className="font-nunito text-[#2D2520]/60 text-lg leading-relaxed max-w-2xl mb-1">
            {tcMeta.subtitle}
          </p>
          <p className="font-nunito text-[#2D2520]/45 text-base mb-4">
            {tcMeta.parenthetical}
          </p>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 font-nunito text-[#2D2520]/40 text-sm">
            <span>{tcMeta.version}</span>
            <span aria-hidden="true">·</span>
            <span>{tcMeta.lastUpdated}</span>
          </div>
        </div>
      </section>
      ...
      <section className="print:hidden mt-16 py-20 text-center bg-[#E8A020] relative overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto px-6 relative z-10">
          ...
          <p className="font-nunito text-white/80 text-lg mb-8">
            We're always happy to talk through anything in this agreement with parents or schools.
          </p>
          <MotionLink
            whileTap={ctaTap}
            to="/contact"
